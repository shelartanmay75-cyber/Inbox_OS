...
export const AuthLayout: React.FC<AuthLayoutProps> = ({ children }) => {
  ...
  // Day/Night toggle placeholder state
  ...
  return (
    <div
      ref={containerRef}
      className="min-h-screen w-full flex flex-col lg:flex-row relative overflow-hidden select-none bg-[#FAF7F2]"
    >
      ...
      <motion.div
        ...
        className="hidden lg:flex lg:w-[58%] flex-col justify-between p-16 relative overflow-hidden bg-gradient-to-br from-white via-[#FAF7F2] to-[#EAE5DA] border-r border-[#EAE5DA] min-h-screen z-10"
      >
        ...
        <motion.div
          ...
          className="absolute top-[18%] right-[8%] w-16 h-16 rounded-[16px] bg-white/40 border border-white/50 backdrop-blur-sm shadow-[0_4px_24px_rgba(0,0,0,0.01)] pointer-events-none"
        />
        <motion.div
          animate={{ y: [0, 16, 0], rotate: [0, -6, 0] }}
          transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut', delay: 1 }}
          className="absolute bottom-[20%] left-[6%] w-24 h-24 rounded-[22px] bg-white/30 border border-white/40 backdrop-blur-md shadow-[0_4px_24px_rgba(0,0,0,0.01)] pointer-events-none"
        />
        ...
        <div className="flex items-center justify-between z-10 relative">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold tracking-tight text-[#1D1D1D] leading-none">
                InboxOS
              </h2>
          ...
          <span className="text-[10px] font-bold px-3 py-1 bg-white border border-[#EAE5DA] text-[#6B7280] rounded-full shadow-sm">
            Open Source
          </span>
        </div>
        ...
        <div className="my-auto space-y-8 max-w-xl text-left z-10 relative">
          {/* Header */}
          <h1 className="text-4xl md:text-[44px] font-extrabold tracking-tight text-[#1D1D1D] leading-[1.1]">
            Your inbox should{' '}
            <span 
          ...
          <div className="bg-white border border-[#EAE5DA] rounded-[22px] p-5 shadow-[0_8px_30px_rgba(0,0,0,0.015)] relative overflow-hidden transition-all hover:shadow-[0_8px_30px_rgba(0,0,0,0.025)]">
            <div className="absolute top-0 right-0 w-24 h-24 bg-[#5F6B38]/3 blur-2xl rounded-full" />
            
            <div className="flex items-center justify-between pb-3 mb-4 border-b border-[#EAE5DA] text-[9px] font-bold uppercase tracking-wider text-[#6B7280]">
            ...
            <div className="grid grid-cols-4 gap-3 relative">
              {[
                { step: 0, icon: <Inbox size={14} strokeWidth={1.5} />, label: 'Ingest', val: 'Parsing body...' },
                { step: 1, icon: <Cpu size={14} strokeWidth={1.5} />, label: 'AI Analyze', val: '94% Urgent' },
                { step: 2, icon: <Zap size={14} strokeWidth={1.5} />, label: 'Rules DSL', val: 'Trigger: Alert' },
                { step: 3, icon: <Send size={14} strokeWidth={1.5} />, label: 'Dispatch', val: 'Sent Slack' },
              ].map((node) => {
                ...
                return (
                  <div
                    ...
                    className={`p-2.5 text-center rounded-[16px] border transition-all duration-300 hover:-translate-y-1 hover:shadow-sm ${
                      isActive
                        ? 'bg-[#5F6B38]/5 border-[#5F6B38]/30 shadow-sm scale-[1.02]'
                        : 'bg-transparent border-[#EAE5DA]'
                    }`}
                  >
                    <div
                      className={`mx-auto h-7 w-7 rounded-lg flex items-center justify-center mb-1.5 transition-colors ${
                        isActive
                          ? 'bg-[#5F6B38] text-white shadow-sm'
                          : isPassed
                          ? 'bg-[#5F6B38]/10 text-[#5F6B38]'
                          : 'bg-[#FAF7F2] text-[#9CA3AF]'
                      }`}
                    >
                      {node.icon}
                    </div>
                    <p className="text-[10px] font-bold text-[#1D1D1D]">
                      {node.label}
                    </p>
                    <p className="text-[8px] truncate mt-0.5 text-[#6B7280] font-mono">
            ...
            <AnimatePresence>
              {pipelineStep === 4 && (
                <motion.div
                  ...
                  exit={{ opacity: 0 }}
                  className="absolute inset-0 bg-white/95 backdrop-blur-sm flex flex-col items-center justify-center p-4 text-center z-20"
                >
                  <div className="h-8 w-8 rounded-full bg-[#5F6B38]/10 flex items-center justify-center mb-2 animate-bounce">
                    <CheckCircle2 size={18} strokeWidth={1.5} className="text-[#5F6B38]" />
                  </div>
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            {featureChips.map((chip, idx) => (
              <motion.span
                ...
                whileHover={{ y: -2 }}
                className="text-[10px] font-bold uppercase tracking-wider px-3.5 py-1.5 bg-[#5F6B38]/5 text-[#5F6B38] rounded-full border border-[#5F6B38]/10 flex items-center gap-1.5 select-none transition-colors hover:bg-[#5F6B38]/10"
              >
        ...
        <div className="w-full border-t border-[#EAE5DA] pt-6 mt-auto z-10 relative">
          <div className="bg-white border border-[#EAE5DA] rounded-[16px] p-4 shadow-[0_4px_20px_rgba(0,0,0,0.01)] max-w-md">
            <div className="min-h-[64px] flex flex-col justify-between text-left relative overflow-hidden">
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className="space-y-2"
                >
                  <p className="text-[11px] italic font-medium leading-relaxed text-[#6B7280]">
                    "{testimonials[activeTestimonial].text}"
                  </p>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-bold text-[#1D1D1D]">
      ...
      {/* ── Right Auth Panel ─────────────────────────────────────────────────────── */}
      <div className="w-full lg:w-[42%] flex flex-col justify-center items-center p-6 md:p-12 relative bg-[#FAF7F2] min-h-screen z-10">
        
        {/* Day/Night Toggle at top right */}
        <div className="absolute top-6 right-6 z-20">
          <button
            onClick={() => setIsDarkMode(!isDarkMode)}
            className="w-10 h-10 rounded-[14px] border border-[#EAE5DA] bg-white text-[#6B7280] hover:text-[#1D1D1D] hover:bg-[#FAF7F2] hover:-translate-y-0.5 active:scale-95 transition-all duration-200 flex items-center justify-center shadow-sm"
        ...
        <motion.div
          ...
          className="w-full max-w-[440px] bg-white border border-[#EAE5DA] rounded-[22px] shadow-[0_8px_30px_rgba(0,0,0,0.015)] p-6 md:p-12 relative flex flex-col"
        >
          {children}
        </motion.div>
      </div>
