...
import { rateLimiter, globalIpRateLimiter } from './middleware/rate-limiter.middleware';
...
const corsOrigin = (origin: string | undefined, callback: (error: Error | null, allow?: boolean) => void) => {
  if (!origin || ALLOWED_ORIGINS.some(o => origin === o) || origin.startsWith('http://localhost:') || origin.startsWith('http://127.0.0.1:')) {
    callback(null, true);
  } else {
