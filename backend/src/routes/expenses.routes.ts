 ...
 */
expensesRouter.get('/', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
 ...
 */
expensesRouter.post('/extract/:emailId', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.userId;
    if (!userId) return res.status(401).json({ error: 'Unauthorized' });
    ...
    logger.info('[Expenses] Extracting expense from email', { emailId, userId });
    const extracted = await ExpenseExtractorService.extractFromEmail(email);

    if (!extracted) {
      return res.status(422).json({ error: 'No expense/receipt data found in this email' });
    }
    ...
    logger.info('[Expenses] Expense extracted and saved', { id: expense.id, userId });
    return res.status(201).json(expense);
  } catch (err: any) {
    logger.error('[Expenses] POST /extract/:emailId error:', err.message);
    return res.status(500).json({ error: 'Failed to extract expense', message: err.message });
  }
});
 ...
 */
expensesRouter.get('/:id', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.userId;
    if (!userId) return res.status(401).json({ error: 'Unauthorized' });
