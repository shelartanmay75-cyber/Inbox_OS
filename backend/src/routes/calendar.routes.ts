 ...
 */
calendarRouter.get('/', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
 ...
 */
calendarRouter.post('/', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.userId;
    if (!userId) return res.status(401).json({ error: 'Unauthorized' });
    ...
    const { emailId, title, startTime, endTime, location, attendees, meetingLink, pushToGoogle } = validation.data;

    // Verify email belongs to user
    const email = await prisma.email.findFirst({ where: { id: emailId as string, userId } });
    if (!email) {
    ...
    if (!startTime || !endTime) {
      ...
      if (!extracted) {
        return res.status(422).json({ error: 'Could not extract event details from email. Please provide startTime and endTime manually.' });
      }
      eventData = extracted;
    } else {
      eventData = {
    ...
    if (pushToGoogle) {
      await calendarEventsQueue.add('createGoogleEvent', { userId, emailId, eventData });
      logger.info('[Calendar] Queued Google Calendar sync job', { userId, emailId });
    }
 ...
 */
calendarRouter.delete('/:id', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.userId;
    if (!userId) return res.status(401).json({ error: 'Unauthorized' });
